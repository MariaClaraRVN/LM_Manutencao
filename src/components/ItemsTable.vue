    ...
    <v-card-text class="pa-2 pa-sm-4">
      ...
      <v-row align="center" dense>
        ...
        <v-col cols="4" sm="2" class="py-1">
          <v-text-field v-model.number="novoItem.quantidade" type="number"
            label="$vuetify.display.xs ? 'Qtd' : 'Quantidade'" variant="outlined" density="compact" min="1"
            :rules="[v => v > 0 || 'Quantidade deve ser maior que zero']"></v-text-field>
        </v-col>

        <!-- Descrição -->
        <v-col cols="8" sm="5" class="py-1">
          <v-text-field v-model="novoItem.descricao" label="Descrição" variant="outlined" density="compact"
            :rules="[v => !!v || 'Descrição é obrigatória']"></v-text-field>
        </v-col>

        <!-- Valor Unitário - esconder label em mobile -->
        <v-col cols="7" sm="3" class="py-1">
          <v-text-field v-model.number="novoItem.valorUnitario" type="number" label="Valor Unitário (R$)"
            variant="outlined" density="compact" min="0" step="0.01" :rules="[]"></v-text-field>
        </v-col>

        <!-- Botão Adicionar - ajustado para mobile -->
        <v-col cols="5" sm="2" class="py-1 d-flex align-center justify-end">
          <v-btn color="primary" @click="adicionarItem"
            :prepend-icon="$vuetify.display.smAndUp ? 'mdi-plus' : undefined"
            :icon="$vuetify.display.xs ? 'mdi-plus' : undefined" size="small" :class="{ 'ml-1': $vuetify.display.xs }"
            :disabled="!novoItem.descricao">
            <span v-if="$vuetify.display.smAndUp">Adicionar</span>
            <span v-else class="sr-only" style="font-size: 25px">+</span>
          </v-btn>
        </v-col>
      </v-row>
      ...
      <div class="mobile-scroll-wrapper mt-3">
            ...
            <tbody>
              <tr v-for="(item, index) in props.itens" :key="index">
                ...
                <td :class="{ 'px-1': $vuetify.display.xs }">
                  <v-text-field v-model.number="item.quantidade" type="number" variant="plain" density="compact"
                    hide-details min="1" class="mobile-input" @input="(e: Event) => {
                ...
                <!-- Descrição -->
                <td :class="{ 'px-1': $vuetify.display.xs }">
                  <v-text-field v-model.string="item.descricao" variant="plain" density="compact" hide-details
                    class="mobile-input" @input="(e: Event) => {
                      const target = e.target as HTMLInputElement;
                ...
                <td class="text-right" :class="{ 'px-1': $vuetify.display.xs }">
                  <v-text-field v-model.number="item.valorUnitario" type="number" variant="plain" density="compact"
                    hide-details min="0" step="0.01" class="text-right mobile-input" @input="(e: Event) => {
                      const target = e.target as HTMLInputElement;
                ...
                <td class="text-center" :class="{ 'px-0': $vuetify.display.xs }">
                  <v-btn density="compact" icon="mdi-delete" color="error" variant="text" size="x-small"
        ...
        <template v-else>
          ...
          <v-alert type="info" density="compact" variant="tonal" text="Nenhum item adicionado. Use o formulário acima."
            class="my-alert-background"></v-alert>
        </template>
      </div>
    </v-card-text>
  </v-card>
...
@media (max-width: 428px) {
  ...
  /* iPhone 14 Pro Max, 12/13 Pro Max */
  .item-table {
    font-size: 0.75rem;
  }
